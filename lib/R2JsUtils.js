	...
	readCsv: function (path) {
        ...
        readVariables: function(path) {
            ...
            lines.forEach(function(line) { 
                ...
                variables.push(splittedString[0]);
            });
            return variables;
        },
        
        // Load variables from file
        loadVariables: function(path, jsVariableStorageObject) {
            var Paths = Java.type('java.nio.file.Paths');
            var Files = Java.type('java.nio.file.Files');
            var lines = Files.readAllLines(Paths.get(path), Java.type('java.nio.charset.StandardCharsets').UTF_8);
            ...
            lines.forEach(function(line) { 
                var splittedString = line.split(":");
                eval(jsVariableStorageObject + "." + splittedString[0] + "=" +splittedString[1]);
            });
            return true;
        },
